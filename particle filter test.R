...
x_out=matrix(0,nrow=1,ncol=time) #系统状态矩阵
x_out[,1]=x #第一个真实状态
...
B_P=matrix(0.1,nrow=1,ncol=time)
#h=1
gamma=matrix(1,nrow=1,ncol=time)
...
for(t in 2:time)
{
  ...
  x_out[,t]=x#x在现实中是未知的，所以下列程序中不应该出现x
  z_out[,t]=z#可用信息 
  ...
  x_estimation[,t]=mean(x_P[1,]) #通过粒子估计真实状态
  
  #h=x_estimation[,t]^2/20+B_P[,t]
  gamma[,t]=gamma[,1]*t^(-0.7)
